...
object MaybeTest {
  def main(args: Array[String]): Unit = {
    ...
    println(s"maybeIncrementedInt: $maybeIncrementedInt")
    println(s"maybeIncrementedInt2: $maybeIncrementedInt2")

    val maybeFiltered = maybeInt.filter(_ % 2 == 0)
    println(s"maybeFiltered: $maybeFiltered")

    val maybeFlatMapped = maybeInt.flatMap(number => Just(number * 3))
    println(s"maybeFlatMapped: $maybeFlatMapped")
  }
}
